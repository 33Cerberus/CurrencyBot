...
Maps the numeric codes returned by the Monobank API to their
alphabetic code and English name.
"""
...
def currency_name(code):
    """Alphabetic code for a numeric code, or the number itself if unknown."""
    entry = CURRENCIES.get(code)
    return entry[0] if entry else str(code)


def currency_full_name(code):
    """Full English name for a numeric code, or None if unknown."""
    entry = CURRENCIES.get(code)
